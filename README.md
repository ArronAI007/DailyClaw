...
[![Version](https://img.shields.io/badge/version-v1.0.0-blue.svg)](https://github.com/ArronAI007/DailyClaw)
...
> 本项目以轻量，易部署为目标
> 
> 本项目基于 [sansan0/TrendRadar](https://github.com/sansan0/TrendRadar) 进行二次开发和功能扩展。
...
### **Web 控制界面（DailyClaw 新增）**

基于 FastAPI 的浏览器配置界面，无需修改配置文件即可管理所有设置：

- **首页新闻卡片流**：打开即看今日新闻，跟真实报告同一条关键词过滤+权重排序链路，支持"换一批"分批浏览，附系统状态与平台状态
- **报告浏览**：按日期分组查看历史 HTML 报告，支持 iframe 嵌入查看
- **平台配置**：可视化添加/删除/修改监控平台，支持自定义每个平台的请求 URL
- **通知配置**：在浏览器中配置企业微信、飞书、钉钉、Telegram、邮件、ntfy、Bark、Slack 等所有推送渠道
- **配置管理**：支持修改爬虫设置、报告模式、权重配置、推送时间窗口、关注词等
- **手动触发**：在 Web 界面一键触发即时爬取

启动方式：`./start-web.sh`（默认端口 18080）
...
**🌐 Web 控制界面（本 Fork 新增功能）**

1. **Dashboard 风格 Web UI**
   - 基于 FastAPI + Jinja2 的现代化控制面板
   - 支持系统状态概览、最新新闻统计、趋势话题、平台状态监控
   - 支持暗黑/亮色主题切换

2. **可视化配置管理**
   - 浏览器中直接修改所有配置，无需编辑 YAML 文件
   - 支持配置爬虫参数、报告模式、权重、推送时间窗口
   - 支持配置所有推送渠道：企业微信、飞书、钉钉、Telegram、邮件、ntfy、Bark、Slack
...
# Slack多账号（2个频道）
SLACK_WEBHOOK_URL=https://example.com/slack-webhook-placeholder;https://hooks.slack.com/yyy
```
